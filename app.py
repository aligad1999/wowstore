    ...
    try:
        ...
        if uploaded_file is not None:
            ...
            if all(column in external_df.columns for column in required_columns):
                st.markdown("📂 **File uploaded and validated successfully!** Loading…")
                ...
                # Merge existing Shopify products with external data
                ...
                if total_operations == 0:
                    st.success("No updates or new products needed.")
                    return
                ...
                for index, row in merged_df.iterrows():
                    ...
                    status_text.text(f"Updating product {completed_operations} of {total_operations}: {row['title']}")
                    ...
                    if not success:
                        logging.warning(f"Failed to update {row['sku']}.")

                    time.sleep(0.3)  # Reduced sleep time for better performance

                # **Create new products**
                for index, row in unmatched_skus.iterrows():
                    completed_operations += 1
                    progress_bar.progress(completed_operations / total_operations)
                    status_text.text(f"Creating product {completed_operations} of {total_operations}: {row['اسم المنتج']}")
                    ...
                    if not result:
                        logging.warning(f"Failed to create product {row['اسم البحث']}.")

                    time.sleep(0.3)  # Optimized for better speed
